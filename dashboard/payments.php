...
// Check if a booking_id was passed in the query string (for "Complete Payment" quick action)
...
?>
  ...
  // Auto-trigger on page load if a booking is pre-selected
  document.addEventListener('DOMContentLoaded', function() {
      var selectEl = document.getElementById('booking_id');
      if (selectEl && selectEl.value) {
          onBookingChange(selectEl);
      }
  });
  </script>
                ...
                <select id="booking_id" name="booking_id" class="form-control" required onchange="onBookingChange(this)">
                    ...
                    <?php foreach ($bookings_list as $b): ?>
                        <option value="<?php echo $b['booking_id']; ?>"
                                <?php echo ($edit_payment['booking_id'] ?? $preselect_booking_id) == $b['booking_id'] ? 'selected' : ''; ?>>
