...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        if ($action === 'create' || $action === 'update') {
            ...
            // Admin override: mark booking as completed without requiring payment
            $id = intval($_POST['booking_id']);
            $stmt = mysqli_prepare($conn, "UPDATE bookings SET status = 'completed' WHERE booking_id = ?");
            mysqli_stmt_bind_param($stmt, 'i', $id);
            if (mysqli_stmt_execute($stmt)) {
                $message = 'Booking marked as completed!';
                $message_type = 'success';
                logActivity('complete', 'booking', $id, []);
            } else {
                $message = 'Error completing booking: ' . mysqli_error($conn);
                $message_type = 'danger';
            }
...
// Get bookings with JOINs and payment totals
$bookings = [];
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <?php if (($b['status'] === 'pending' || $b['status'] === 'confirmed') && ($b['total_amount'] > $b['total_paid'])): ?>
                                        <a href="<?php echo $base_url; ?>/dashboard/payments.php?action=new&booking_id=<?php echo $b['booking_id']; ?>"
                                           class="btn btn-sm btn-primary" title="Record Payment & Complete">
                                            ✓ Finish Pending
                                        </a>
                                    <?php endif; ?>
                                    <?php if ($b['status'] === 'pending' || $b['status'] === 'confirmed'): ?>
                                        <form method="POST" style="display:inline;"
                                              onsubmit="return confirm('Mark this booking as COMPLETED without payment? Only use for admin overrides.');">
                                            <input type="hidden" name="action" value="complete">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                                            <input type="hidden" name="booking_id" value="<?php echo $b['booking_id']; ?>">
                                            <button type="submit" class="btn btn-sm btn-success" title="Mark Complete (Admin Override)">
                                                ✓ Mark Complete
                                            </button>
                                        </form>
                                    <?php endif; ?>
