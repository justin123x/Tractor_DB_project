...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        if ($action === 'delete') {
            $id = intval($_POST['tractor_id'] ?? 0);
            
            // Validate ID
            if ($id <= 0) {
                $message = '❌ Invalid tractor ID (received: ' . htmlspecialchars($_POST['tractor_id'] ?? 'none') . ')';
                $message_type = 'danger';
            } else {
                // Verify tractor exists before delete
                $exists_check = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM tractors WHERE tractor_id = $id");
                $exists = mysqli_fetch_assoc($exists_check);
                if ($exists['cnt'] == 0) {
                    $message = 'Tractor not found. It may have already been deleted.';
                    $message_type = 'warning';
                } else {
                    // Get tractor name and image for cleanup/logging
                    $tractor_name = '';
                    $name_res = mysqli_query($conn, "SELECT tractor_name, image_path FROM tractors WHERE tractor_id = $id");
                    $tractor_data = mysqli_fetch_assoc($name_res);
                    if ($tractor_data) {
                        $tractor_name = $tractor_data['tractor_name'];
                        $image_path = $tractor_data['image_path'];
                    ...
                    if (mysqli_stmt_execute($stmt)) {
                        ...
                        if ($affected > 0) {
                            $message = "Tractor '$tractor_name' deleted successfully! (ID: $id)";
                            $message_type = 'success';
                            logActivity('delete', 'tractor', $id, ['name' => $tractor_name]);
                        } else {
                            $message = "Delete query succeeded but no rows were affected. Tractor ID: $id may not exist.";
                            $message_type = 'warning';
                        }
                    } else {
...
<div class="main-content dashboard-main">
    ...
    <div class="page-header">
        <h1><span class="icon">🚜</span> Tractors</h1>
        <div class="d-flex gap-2">
            <?php if ($show_form): ?>
                <a href="<?php echo $base_url; ?>/dashboard/tractors.php" class="btn btn-secondary">← Back to List</a>
            <?php else: ?>
    ...
    <div class="form-container animate-fade-in" style="max-width: 700px;">
        <h2 style="margin-bottom: 1.5rem; font-size: 1.3rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;">
            <?php echo $edit_tractor ? '✏️ Edit Tractor' : '➕ Add New Tractor'; ?>
        </h2>
        <form method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <?php if ($edit_tractor && !empty($edit_tractor['image_path'])): ?>
                    <div class="image-preview mt-2" id="existingImagePreview">
                        ...
                        <small class="text-muted" style="display:block; margin-top:4px;">Current image — upload a new one to replace it</small>
                    </div>
                <?php endif; ?>
            ...
            <div class="d-flex gap-2 mt-3">
                <button type="submit" class="btn btn-primary">
                    <?php echo $edit_tractor ? '💾 Update Tractor' : '➕ Add Tractor'; ?>
                </button>
            ...
            <?php foreach ($tractors as $tractor): ?>
                ...
                <div class="tractor-card">
                    ...
                    <div class="tractor-card-body">
                        ...
                        <?php if (!empty($tractor['description'])): ?>
                            <div class="tractor-description" style="margin-top: 0.5rem; font-size: 0.85rem; color: var(--text-muted, #999); line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;">
                                <?php echo htmlspecialchars($tractor['description']); ?>
                            </div>
                        <?php endif; ?>
                        <div class="tractor-specs">
                            <div class="spec">
                                <div class="spec-value"><?php echo $tractor['horsepower'] ?? '—'; ?></div>
                                <div class="spec-label">HP</div>
                            </div>
                            <div class="spec">
                                <div class="spec-value"><?php echo $tractor['year_manufactured'] ?? '—'; ?></div>
